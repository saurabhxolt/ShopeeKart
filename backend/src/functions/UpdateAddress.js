...
app.http('UpdateAddress', {
    ...
    handler: async (request, context) => {
        try {
            ...
            if (!data.addressId || !data.userId || !data.fullName || !data.addressLine) {
            ...
            const query = `
                ...
                SET FullName = @fullName, Phone = @phone, Pincode = @pincode, 
                    AddressLine = @addressLine, City = @city, District = @district, State = @state
            ...
            return { status: 200, jsonBody: { message: "Address updated successfully!" } };
        } catch (err) {
            context.error(err);
            return { status: 500, body: "Error updating address: " + err.message };
        }
    }
});