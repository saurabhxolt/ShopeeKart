...
const SellerOrdersView = ({ sellerId, isMobile }) => {
    ...
    const fetchCatalog = useCallback(async () => {
        if (!sellerId) return;
        try {
            const res = await axios.get(`http://localhost:7071/api/GetSellerProducts?userId=${sellerId}`);
            setCatalog(res.data);
        } catch (err) {
    ...
    }, [sellerId, fetchOrders, fetchCatalog]);
        ...
        <div style={{ width: '100%', boxSizing: 'border-box' }}>
            ...
            <div style={{ width: '100%', boxSizing: 'border-box' }}>
                {loading ? (
                    ...
                    // Parse Address and Payment Method early so it can be used in the header bar
                    ...
                    return (
                        <div key={o.OrderId} style={{ border: '1px solid #e0e0e0', borderRadius: '12px', marginBottom: '15px', background: o.Status?.includes('Cancelled') ? '#fffcfc' : '#fff', boxShadow: '0 4px 10px rgba(0,0,0,0.03)', width: '100%', boxSizing: 'border-box', overflow: 'hidden' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '15px', width: '100%' }}>
                                    ...
                                    <div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                                            <h4 style={{ margin: '0', fontSize: '18px' }}>Order <span style={{ color: '#007bff' }}>#{o.OrderId}</span></h4>
                                            {/* 🔥 Payment Mode Badge in Bar */}
                                            <span style={{ background: paymentMethod === 'COD' ? '#fff3cd' : '#d1e7dd', color: paymentMethod === 'COD' ? '#856404' : '#0f5132', padding: '2px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold', border: '1px solid rgba(0,0,0,0.1)' }}>
                                                {paymentMethod}
                                            </span>
                                        </div>
                                        <div style={{ fontSize: '13px', color: '#666', display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center', marginTop: '5px' }}>
                            ...
                            {/* EXPANDABLE BODY */}
                            {isExpanded && (
                                <div style={{ padding: isMobile ? '0 15px 15px 15px' : '0 20px 20px 20px', borderTop: '1px solid #eee', marginTop: '5px', paddingTop: '15px' }}>
                                    <div style={{ background: '#f8f9fa', padding: '15px', borderRadius: '8px', marginBottom: '20px', border: '1px solid #e9ecef', fontSize: '14px', color: '#333', width: '100%', boxSizing: 'border-box' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', borderBottom: '1px solid #dee2e6', paddingBottom: '10px' }}>
                                            <span style={{ fontWeight: 'bold', color: '#495057', fontSize: isMobile ? '13px' : '14px' }}>👤 Customer Info & Payment Details</span>
                                        </div>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: '15px', width: '100%' }}>
                                            <div style={{ flex: 1, paddingBottom: isMobile ? '10px' : '0', borderBottom: isMobile ? '1px dashed #ddd' : 'none' }}>
                                                <div style={{ marginBottom: '12px' }}>
                                                    <span style={{ color: '#6c757d', fontSize: '11px', display: 'block', textTransform: 'uppercase' }}>Buyer Name</span> 
                                                    <strong style={{ wordBreak: 'break-word' }}>{o.BuyerName || "Unknown"}</strong>
                                                </div>
                                                {/* 🔥 Transaction ID shown here */}
                                                <div style={{ marginBottom: '12px' }}>
                                                    <span style={{ color: '#6c757d', fontSize: '11px', display: 'block', textTransform: 'uppercase' }}>Transaction ID</span> 
                                                    <strong style={{ wordBreak: 'break-word', color: '#007bff' }}>{o.TransactionId || o.TxnId || "N/A (Cash on Delivery)"}</strong>
                                                </div>
                                                <div>
                                                    <span style={{ color: '#6c757d', fontSize: '11px', display: 'block', textTransform: 'uppercase' }}>Email Contact</span> 
                                                    <a href={`mailto:${o.BuyerEmail}`} style={{ color: '#007bff', textDecoration: 'none', fontWeight: 'bold', wordBreak: 'break-all' }}>{o.BuyerEmail || "N/A"}</a>
                                                </div>
                                            </div>
            ...
            {/* PRODUCT DETAILS MODAL remains same... */}
            {viewingItem && (() => {
                ...
                return (
                    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: isMobile ? 'flex-end' : 'center', zIndex: 9999, padding: isMobile ? '0' : '20px' }}>
                        <div style={{ backgroundColor: 'white', padding: isMobile ? '20px' : '30px', borderRadius: isMobile ? '16px 16px 0 0' : '12px', width: '100%', maxWidth: '550px', maxHeight: '90vh', overflowY: 'auto', position: 'relative', boxShadow: '0 -10px 30px rgba(0,0,0,0.5)', boxSizing: 'border-box' }}>
                            ...
                            <div style={{ textAlign: 'center', marginBottom: '20px', background: '#f8f9fa', borderRadius: '8px', padding: '15px', border: '1px solid #e9ecef', width: '100%', boxSizing: 'border-box' }}>
                                <img src={parseImages(viewingItem.ImageUrl)[0]} alt="Product" style={{ maxWidth: '100%', maxHeight: '200px', objectFit: 'contain', borderRadius: '4px' }} />
                            </div>
                            ...
                            <button onClick={() => setViewingItem(null)} style={{ width: '100%', padding: '16px', background: '#343a40', color: 'white', border: 'none', borderRadius: '8px', marginTop: '20px', cursor: 'pointer', fontWeight: 'bold', fontSize: '16px', transition: 'background 0.2s', boxSizing: 'border-box' }}>Close Details</button>
                        </div>
                    </div>
                );
            })()}
